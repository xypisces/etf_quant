...
- **AND** 当前 bar 的成交量 < 过去 `volume_period` 日的简单成交量均值
...
#### Scenario: 生成买入信号
- **WHEN** 价格曾回踩 EMA20 附近（收盘价 ≤ EMA20 × (1 + tolerance)）后重返 EMA20 上方
...
#### Scenario: 回踩容忍度可配置
- **WHEN** 创建策略实例
- **THEN** 必须（SHALL）支持配置 `pullback_tolerance` 参数，默认为 0.005（0.5%）
